...
**The rule:** folder names must **scream the domain** (Martin, *Screaming Architecture*), and you **adopt a battle-tested taxonomy** rather than coining your own — Feature-Sliced Design or Bulletproof React for JS/TS; Ports & Adapters, domain services, and the Shared Kernel (DDD / Hexagonal) for the cross-cutting core. Name each slice by the **adopted taxonomy's convention** — do not hardcode one: the feature-slice taxonomies (FSD, Bulletproof React) name slices by **domain noun** (`posts`, `discussions`, `users`) — the common case; CQRS vertical slices (Bogard) name by **use-case/verb** (`create-order`). The shared kernel gets a **domain** name (`core`, `domain`, `entities`). Never a mechanism name — pick a taxonomy, then be consistent within it. Imports flow one way (down into the kernel and shared, never sideways between slices), and you **enforce the taxonomy with a linter** (Steiger; `eslint import/no-restricted-paths`) — an un-enforced convention erodes within weeks.
