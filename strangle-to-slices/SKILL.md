...
**The rule:** "make the change easy, then make the easy change" (Beck; Fowler). Carve the slice you are **about to build a feature in or fix a bug in** — the migration is the *make-it-easy* step for real, imminent work, not a standalone cleanup (the workflow Fowler flags as hardest to justify). Value ships with every step because every step is in service of shippable work. To make it self-propelling, **mandate the new structure for all new features** — new work is born sliced, so the layered code shrinks whether or not you ever run a dedicated pass. Make that mandate concrete by migrating **one feature fully and correctly first, as the exemplar** — the reference new work copies *and* the proven template (and runbook) for migrating the rest. Order matters — choose the first slice by three tests, not by size: (1) **representative** — it goes through the hard parts the other slices share (a route, background jobs, the tenant/security fences); one that skips them teaches nothing about the rest; (2) **pinnable** — it has a stable entry point (a single event or function, not an untested HTTP route) where you can characterize behavior before and after the move; (3) **rides real work** — ideally a bug or feature is already forcing you to touch it, so the move isn't busywork. Prefer representative-with-a-pin over merely-small-and-safe. Prove the pattern there, then sequence the rest value/risk-first (Principle 6) or on-touch.
...
**The rule:** extract a **minimal, dependency-free shared kernel first** (framework bootstrap, cross-cutting infra, core domain types) that slices sit on and that imports nothing above it (Comartin; FSD). Then, **every iteration**, push anything now used by only one slice *back down into that slice* — depopulating the shared layer is a continuous defense, not an end-of-project cleanup. Prefer duplication over premature abstraction; promote to the kernel only on the third occurrence, once the shape is stable (Rule of Three). Evolve shared kernel types with **expand / migrate / contract** (Fowler *Parallel Change*) — never break all consumers at once. And **contract aggressively**: after a slice bakes at 100%, delete the old layered path, the shadow comparator, and the toggle, and collapse the abstraction if it no longer earns its keep — deleting legacy is a deliberate **final** step per feature, after which rollback cost jumps. Enforce boundaries mechanically with an **architectural fitness function** (Ford, Parsons & Kua) — an automated boundary check, e.g. an FSD linter like Steiger: migrated slices set to error, legacy to warn, tightening as you go — an unguarded half-migration silently drifts back. **Guard the guard:** a fitness function keyed on **path literals** — a hardcoded path regex, or a CI trigger naming a specific file — silently rots the moment code moves, then reports **green while dead**. Assert that every inclusion-scoped rule matched **≥1 file** this run; a rule matching zero files is a broken fence, not a passing one (the mutation-check of Principle 3, aimed at the guard itself). Prefer content/AST triggers over path literals. And name each extracted slice **by intent, per an established taxonomy** (`features/` named by the adopted taxonomy — a domain noun in FSD/Bulletproof React, a use-case verb in CQRS/Bogard — a `core` / `domain` kernel; never a mechanism name like "engine" or "utils"): the migration target *is* the named structure the sister skill `vertical-slicing` (Principle 9) defines, so migrate toward those names, not toward a new dumping ground.
...
- **Ground truth is the code, traced until clear — not the docs.** The source code is the real design (Reeves, "What Is Software Design?", 1992), and *"truth can only be found in one place: the code"* (Martin, *Clean Code*, 2008). Docs, ADRs, and comments **drift** — "inaccurate comments are far worse than no comments at all" — so treat them as unverified leads at most: confirm every claim against the code, never as authority, since a stale one mis-binds the whole migration. Derive every field above from the codebase itself (config, build/launch scripts, and dependency behavior included), tracing each thread until the fact is settled. Before "fixing" something that looks wrong or missing, apply **Chesterton's Fence** (Chesterton, *The Thing*, 1929) — don't tear out or bolt on a guard until you know why it's absent; the omission is often deliberate, required by a build/launch path — and enforce only what the repo's own fitness function actually enforces.
...
- **Neal Ford, Rebecca Parsons & Patrick Kua** — *Building Evolutionary Architectures* (O'Reilly, 1st ed. 2017; the 2nd ed., 2022, *Automated Software Governance*, adds Pramod Sadalage): the **architectural fitness function** — "an objective integrity assessment of some architectural characteristic(s)" — adapted to software architecture from the **fitness function** of evolutionary computation / genetic algorithms (orig. **John Holland**, *Adaptation in Natural and Artificial Systems*, 1975). An automated boundary linter such as **Steiger** (Feature-Sliced Design) is one implementation; describing such a linter as a "fitness function" is this skill's framing, not the tool's own.
